...
- **Docker / GHCR**: [`Dockerfile`](Dockerfile) serves static `dist/` via nginx. [`docker-compose.yaml`](docker-compose.yaml) is image-only for Portainer; [`docker-compose.build.yaml`](docker-compose.build.yaml) builds locally. Releases publish to GHCR via [`.github/workflows/release-ghcr.yml`](.github/workflows/release-ghcr.yml).
